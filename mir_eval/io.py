...
def load_events(filename, delimiter='\t', converter=None, label_prefix='__'):
    '''Import time-stamp events from an annotation file.  This is primarily
    useful for processing events which lack duration, such as beats or onsets.

    The annotation file may be either of two formats:
      - Single-column.  Each line contains a single value, corresponding to the
        time of the annotated event.

      - Double-column.  Each line contains two values, separated by
        ``delimiter``: the first contains the time of the annotated event, and
        the second contains its label.
...
def load_annotation(filename,
                    ...
                    converter=None,
                    label_prefix='__'):
    '''Import annotation events from an annotation file.  This is primarily
    useful for processing events which span a duration, such as segmentation,
    chords, or instrument activation.

    The annotation file may be either of two formats:
      - Double-column.  Each line contains two values, separated by
        ``delimiter``, corresponding to the start and end time annotated event.

      - Triple-column.  Each line contains three values, separated by
        ``delimiter``.  The first two values specify the start and end times,
        the last value specifies the label for the event (e.g. "Verse" or
        "A:min").
