...
if __name__ == '__main__':
    ...
    # Load in data
    reference_tempi = mir_eval.io.load_delimited(parameters['reference_file'], [float]*2)
    estimated_tempi = mir_eval.io.load_delimited(parameters['estimated_file'], [float]*3)

    reference_tempi = np.concatenate(reference_tempi)
    estimated_weight = estimated_tempi[-1]
    estimated_tempi = np.concatenate(estimated_tempi[:2])

    # Compute all the scores
    scores = mir_eval.tempo.evaluate(reference_tempi, estimated_tempi, estimated_weight)
